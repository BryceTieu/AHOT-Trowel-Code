...
public class TrowelTeleOp extends OpMode {
    ...
    // Camera auto-aim: use AprilTag bearing as heading error.
    // LB toggles auto-aim. If no valid tag is seen, driver keeps manual turn control.

    // Field coordinates used for odometry display: (0,0) is bottom-left, X right, Y up.

    // Tunable constants live in TrowelConfigs (dashboard-editable).
    ...
    private double headingErrorIntegral  = 0.0;
    private double lastTurnUpdateTime    = 0.0;
    ...
    private AutoAimState autoAimState   = AutoAimState.DISABLED;

    // Runtime state.

    // Current auto-aim target data (used for control + telemetry).
    private boolean autoAimHasTarget     = false;
    private double  autoAimBearing       = 0.0;
    private int     autoAimTagId         = -1;
    private double  autoAimTagRange      = 0.0;
    ...
    private boolean prevAutoAimBtn     = false;
    ...
    // Closest AprilTag seen this loop (independent of auto-aim).
    ...
    public void init() {
        ...
        boolean vOK = robot.initVision(TrowelConfigs.WEBCAM_NAME,
                TrowelConfigs.CAMERA_X_OFFSET_INCHES, TrowelConfigs.CAMERA_Y_OFFSET_INCHES, TrowelConfigs.CAMERA_Z_OFFSET_INCHES,
                TrowelConfigs.CAMERA_YAW_DEG, TrowelConfigs.CAMERA_PITCH_DEG, TrowelConfigs.CAMERA_ROLL_DEG);
        ...
        if (robot.transferServo != null) {
            robot.transferServo.setPosition(TrowelConfigs.SERVO_IDLE);
        }
    ...
    public void init_loop() {
        ...
        if (initialPoseEstablished) {
            telemetry.addData("Pose Status", "Initial pose set from camera");
            if (odometry != null) {
                Odometry.Position p = odometry.getPosition();
                telemetry.addData("Odometry Reports", "X: %.1f in, Y: %.1f in, H: %.1f deg",
                        p.xMm / 25.4, p.yMm / 25.4, Math.toDegrees(p.headingRad));
            }
            if (lastCameraPoseX != 0 || lastCameraPoseY != 0) {
                telemetry.addData("Camera Calculated", "X: %.1f in, Y: %.1f in, H: %.1f deg",
    ...
    /**
     * Try to set the robot's starting pose from AprilTag camera data.
     */
    private void attemptInitialPoseFromCamera() {
        ...
        // If camera lock fails for too long, use an alliance-based fallback pose.
    ...
    /**
     * Rebuild robot XY from known tag field position + camera range/bearing
     * when SDK robotPose is out of bounds.
     *
     * @return {x, y} in inches, or null if we cannot resolve the tag field position.
     */
    ...
    private void showControls() {
        ...
        telemetry.addLine("  LB press: Toggle Auto-Aim");
        telemetry.addLine("  LT pull: Shoot (servo 0.85)");
        telemetry.addLine("");
    ...
    public void loop() {
        ...
        // Keep odometry synced to camera when valid tag data is available.
    ...
    // Track closest visible AprilTag every loop.
    ...
    // Apply camera pose corrections to odometry (separate from aiming).
    ...
    // Driver control + optional auto-aim.

    private void updateDrive() {
        isSlowMode = gamepad1.right_bumper;

        // Toggle auto-aim with left bumper.
        boolean autoAimBtn = gamepad1.left_bumper;
        if (autoAimBtn && !prevAutoAimBtn) {
            if (autoAimState == AutoAimState.DISABLED) {
                autoAimState = AutoAimState.ACTIVE;
                resetTurnController();
            } else {
                stopAutoAim();
            }
        }
        prevAutoAimBtn = autoAimBtn;

        if (autoAimState == AutoAimState.ACTIVE) {
            updateAutoAim();
        } else {
            updateManualDrive();
        }
    ...
    private void stopAutoAim() {
        autoAimState = AutoAimState.DISABLED;
        resetTurnController();
        autoAimHasTarget = false;
        applyDrivePower(0, 0, 0, 0);
    }

    private void resetTurnController() {
        lastHeadingError     = 0.0;
        headingErrorIntegral = 0.0;
        lastTurnUpdateTime   = loopTimer.seconds();
    }

    // Camera-based auto-aim:
    // pick the closest valid alliance tag and PID-turn from its bearing.
    private void updateAutoAim() {
        // Reset target selection for this loop.
        autoAimHasTarget = false;
        autoAimBearing   = 0;
        autoAimTagId     = -1;
        autoAimTagRange  = 0;
        ...
        if (robot.isVisionEnabled()) {
            ...
            for (AprilTagDetection det : detections) {
                ...
                // Ignore tags that do not match selected alliance.
                if (selectedAlliance == Alliance.BLUE
                        && !(det.id >= TrowelConfigs.BLUE_TAG_ID_MIN && det.id <= TrowelConfigs.BLUE_TAG_ID_MAX)) continue;
                if (selectedAlliance == Alliance.RED
                        && !(det.id >= TrowelConfigs.RED_TAG_ID_MIN && det.id <= TrowelConfigs.RED_TAG_ID_MAX)) continue;

                if (det.ftcPose.range < bestRange) {
                    bestRange        = det.ftcPose.range;
                    autoAimHasTarget = true;
                    autoAimBearing   = det.ftcPose.bearing;
                    autoAimTagId     = det.id;
                    autoAimTagRange  = det.ftcPose.range;
                }
            }
        }

        if (!autoAimHasTarget) {
            // No target: keep full manual drive, but stay in auto-aim mode.
            updateManualDrive();
            return;
        }

        // Tag bearing is the heading error fed into turn PID.
        double turnPower = calculateTurnPower(autoAimBearing);

        applyAutoAimDrive(turnPower);
    }

    /**
     * Drive translation stays on left stick while PID controls rotation.
     */
    private void applyAutoAimDrive(double turnPower) {
        ...
        if (isSlowMode) {
            fwd *= TrowelConfigs.SLOW_MODE_MULTIPLIER;
            str *= TrowelConfigs.SLOW_MODE_MULTIPLIER;
        }

        double fl = fwd + str + turnPower;
        double fr = fwd - str - turnPower;
        double bl = fwd - str + turnPower;
        double br = fwd + str - turnPower;
    ...
    private void updateManualDrive() {
        ...
        if (isSlowMode) {
            fwd *= TrowelConfigs.SLOW_MODE_MULTIPLIER;
            str *= TrowelConfigs.SLOW_MODE_MULTIPLIER;
            rot *= TrowelConfigs.SLOW_MODE_MULTIPLIER;
        }
    ...
    // Turn PID using constants from TrowelConfigs.
    // Input: heading error in degrees (positive turns right).
    // Output: clamped turn power.

    private double calculateTurnPower(double headingError) {
        double currentTime = loopTimer.seconds();
        double dt = lastTurnUpdateTime > 0 ? (currentTime - lastTurnUpdateTime) : 0.016;
        dt = Math.min(Math.max(dt, 0.001), 0.1);
        lastTurnUpdateTime = currentTime;

        // Proportional term.
        double pTerm = headingError * TrowelConfigs.HEADING_P;

        // Integral term with anti-windup.
        if (Math.abs(headingError) < TrowelConfigs.INTEGRAL_ACTIVE_THRESHOLD_DEG) {
            headingErrorIntegral += headingError * dt;
            headingErrorIntegral = Math.max(-TrowelConfigs.INTEGRAL_MAX_ACCUMULATION,
                    Math.min(TrowelConfigs.INTEGRAL_MAX_ACCUMULATION, headingErrorIntegral));
        } else {
            headingErrorIntegral = 0;
        }
        double iTerm = headingErrorIntegral * TrowelConfigs.HEADING_I;

        // Derivative term.
        double errorRate = (headingError - lastHeadingError) / dt;
        double dTerm = errorRate * TrowelConfigs.HEADING_D;

        // Feed-forward term (currently near zero).
        double fTerm = TrowelConfigs.HEADING_F * TrowelConfigs.HEADING_F_SCALE;

        // Sum and clamp output.
        double turnPower = pTerm + iTerm + dTerm + fTerm;
        turnPower = Math.max(-TrowelConfigs.MAX_TURN_POWER, Math.min(TrowelConfigs.MAX_TURN_POWER, turnPower));

        // Apply minimum output to overcome static friction.
        if (Math.abs(turnPower) > 0.01 && Math.abs(turnPower) < TrowelConfigs.MIN_TURN_POWER) {
            turnPower = Math.signum(turnPower) * TrowelConfigs.MIN_TURN_POWER;
        }
    ...
    private void updateFlywheel() {
        ...
        updateFlywheelCommon(gamepad1.left_trigger > 0.5);
    }
    ...
    private void updateFlywheelCommon(boolean shooting) {
        ...
        double exactTarget = flywheelTarget + (shooting ? TrowelConfigs.SHOOT_BOOST : 0);
    ...
    private void updateIntake() {
        double desired1;
        if      (gamepad2.left_trigger  > 0.3) desired1 =  TrowelConfigs.INTAKE_POWER;
        else if (gamepad2.right_trigger > 0.3) desired1 = -TrowelConfigs.INTAKE_POWER;
        else                                   desired1 =  0;

        double desired2;
        if      (gamepad2.a) desired2 =  TrowelConfigs.INTAKE_POWER;
        else if (gamepad2.b) desired2 = -TrowelConfigs.INTAKE_POWER;
        else                 desired2 =  0;
    ...
    private void updateServo() {
        if (robot != null && robot.transferServo != null) {
            try {
                robot.transferServo.setPosition(
                        gamepad1.left_trigger > 0.5 ? TrowelConfigs.SERVO_SHOOT : TrowelConfigs.SERVO_IDLE);
            } catch (Exception e) {
                telemetry.addData("Servo Error", e.getMessage());
            }
        }
    }
    ...
    private void updateTelemetry() {
        ...
        telemetry.addData("Pose Init", initialPoseEstablished ? "YES" : "NO");
        telemetry.addLine("");

        // Auto-aim status.
        telemetry.addLine("AUTO-AIM (Camera Only)");
        if (autoAimState == AutoAimState.DISABLED) {
            telemetry.addData("Status", "OFF (LB to toggle)");
        } else if (!autoAimHasTarget) {
            telemetry.addData("Status", "SEARCHING - no tag (manual control)");
        } else if (Math.abs(autoAimBearing) <= TrowelConfigs.AIM_LOCK_TOLERANCE_DEG) {
            telemetry.addData("Status", "LOCKED bearing %.1f deg", autoAimBearing);
            telemetry.addData("  Tag", "#%d  %.1f in away", autoAimTagId, autoAimTagRange);
        } else {
            telemetry.addData("Status", "AIMING bearing %.1f deg", autoAimBearing);
            telemetry.addData("  Tag", "#%d  %.1f in away", autoAimTagId, autoAimTagRange);
        }
        telemetry.addLine("");
        ...
        if (tagDetected)
            telemetry.addData("Closest Tag", "#%d  %.1f in  bearing %.1f deg",
        ...
        if (autoAimState == AutoAimState.ACTIVE) modeStr += " + AUTO-AIM";
        ...
        if (odoEnabled && odometry != null) {
            ...
            telemetry.addData("Heading", "%.1f deg", Math.toDegrees(p.headingRad));
            ...
            if (lastCameraPoseX != 0 || lastCameraPoseY != 0)
                telemetry.addData("Last Cam Pose", "X:%.1f Y:%.1f H:%.1f deg",
                        lastCameraPoseX, lastCameraPoseY, lastCameraPoseHeading);
        } else {
